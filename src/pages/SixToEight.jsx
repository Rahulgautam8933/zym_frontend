...
const SixToEight = () => {
  ...
  const getdata = async (pageNumber) => {
    try {
      const res = await axios.get(
        ...
        }/getdatasixToEight?page=${currentPage}&searchQuery=${query}`
      );
  ...
  const handleEdit = (id) => {
    ...
    navigator(`/sixToEight/updateData/${id}`);
  };

  const handleUser = (id) => {
    // Handle edit logic with the ID

    navigator(`/sixToEight/userData/${id}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      const res = await axios.delete(
        `${import.meta.env.VITE_APP_BASE_URL}/deleteusersixToEight/${deleteId}`
      );
      ...
      <div className="container my-4">
        ...
        <div className="row my-4">
          ...
          <div className="col text-end">
            <Link to="/sixToEight/addData" className="btn btn-primary">
              + Add Student Data
            </Link>
          </div>
          <div className="col text-end">
